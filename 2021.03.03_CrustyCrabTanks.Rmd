...
dFishCareUDA <- read.csv(here("01_rawData", "2021.02.18_FishCareLog_salinity.csv")) 
dim(dFishCareUDA)
#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
# dim(dShedUDA)
# Transformed File
dShedUDA <- read.csv(here("01_rawData", "2021.02.18_pH_Salinity_UDA.csv"))
dim(dShedUDA)

#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |

dHOBOppt <- read.csv(here("01_rawData","2021.02.18b_dHOBOlogCNDCT.csv"))
dim(dHOBOppt)
...
dFishCareUDA$location[dFishCareUDA$location == "salmonShed" ] <- "ambient"
dFishCareUDA$location[dFishCareUDA$location == "chumHouse" ] <- "ambient_chumhouse"
...
dFishCareUDA$salinity <- rename(dFishCareUDA, Salinity = salinity)
dFishCareUDA <- dFishCareUDA %>% rename(Salinity = salinity) 
...
HOBOSalinityProbe_plot <- ggplot(dHOBOppt, aes(ObservationDate, salinityPPT)) + 
  geom_point() +
  ylim(24,30) +
