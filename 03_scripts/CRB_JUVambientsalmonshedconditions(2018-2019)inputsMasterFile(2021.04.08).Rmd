...
write.csv(wider_temp, "CRB_JUV_SalmonShed_temp_summaryWIDE_2021.04.08.csv")
...
names(SalmonShed_pH_summary)
# [1] "ObservationMonth" "treatment"        "sensortype"       "sd"              
# [5] "mean"             "n"                "se"               "ci"  
names(pretty_wider_pH)
# [1] "ObservationMonth"      "treatment"             "mean_OceanOptics1.6.7"
# [4] "mean_UDA2182durfet"    "sd_OceanOptics1.6.7"   "sd_UDA2182durfet"     
# [7] "se_OceanOptics1.6.7"   "se_UDA2182durfet"      "pHrange" 
...
pretty_wider_pH <- SalmonShed_pH_summary %>% select(ObservationMonth, treatment, sensortype, mean, sd, se) %>% pivot_wider(names_from = c(sensortype), values_from = c(mean, sd, se)) %>%
  mutate(SPECpHrange = paste(round(mean_OceanOptics1.6.7,2), " (", round(se_OceanOptics1.6.7,2),")")) %>% mutate(SPECpHrange = paste(round(mean_OceanOptics1.6.7,2), " (", round(mean_OceanOptics1.6.7-1.96 *se_OceanOptics1.6.7, 2), "-", round(mean_OceanOptics1.6.7+1.96 *se_OceanOptics1.6.7, 2), ")"))  %>%
  mutate(UDApHrange = paste(round(mean_UDA2182durfet,2), " (", round(se_UDA2182durfet,2),")")) %>% mutate(UDApHrange = paste(round(mean_UDA2182durfet,2), " (", round(mean_UDA2182durfet-1.96 *se_UDA2182durfet, 2), "-", round(mean_UDA2182durfet+1.96 *se_UDA2182durfet, 2), ")"))
...
# SalmonShed_salinity_summary
pretty_wider_salinity <- SalmonShed_salinity_summary %>% select(ObservationMonth, sensortype, mean, sd, se) %>% pivot_wider(names_from = c(sensortype), values_from = c(mean, sd, se), names_glue = "{sensortype}: {.value}")

names(pretty_wider_salinity)
# [1] "ObservationMonth"       "OrionStarA112: mean"    "SENSOREXtorodial: mean"
# [4] "OrionStarA112: sd"      "OrionStarA112: se"   "OrionStarA112: se"     
# [7] "SENSOREXtorodial: se"  

pretty_wider_salinity <- SalmonShed_salinity_summary %>% select(ObservationMonth, sensortype, mean, sd, se) %>% pivot_wider(names_from = c(sensortype), values_from = c(mean, sd, se)) %>%
   mutate(salintyRange = paste(round(OrionStarA112: mean,2), " (", round(OrionStarA112: se,2),")")) %>% mutate(salinityRange = paste(round(OrionStarA112: mean,2), " (", round(OrionStarA112: mean-1.96 *OrionStarA112: se, 2), "-", round(mean+1.96 *OrionStarA112: se, 2), ")"))
...
names(SalmonShed_salinity_summary)
# [1] "ObservationMonth" "sensortype"       "sd"               "mean"            
# [5] "n"                "se"               "ci"    
...
names(wider_temp)
# [1] "ObservationMonth"    "UDA2182: mean"       "20449374: mean"      "OrionStarA112: mean"
#  [5] "UDA2182: sd"         "20449374: sd"        "OrionStarA112: sd"   "UDA2182: se"        
#  [9] "20449374: se"        "OrionStarA112: se"   
